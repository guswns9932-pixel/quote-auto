...
WHATS_NEW = [
    ...
    },
    {
        ...
        "desc": "PDF 저장이 백그라운드에서 진행돼 더는 창이 멈추지 않고, "
                ...
                "자동으로 채워지고, 승인코드 로드 상태와 서명 미리보기를 "
                "바로 확인할 수 있습니다.",
    },
    {
        ...
        "desc": "Credit·보증기간·투자자·견적서 타입을 항목별 열로 나누고, "
                ...
                "투자자, 국내/중국/미국)를 바로 표시합니다. 작업로그창을 "
                "줄이고 그만큼 옵션 패널을 넓혔습니다.",
    },
    {
    ...
    },
    {
        ...
        "app": "csp",
        "title": "FSC 추천 기능 추가",
        "desc": "옵션(사업장·DEVICE·대공정·설비사·세부공정·Q-code)이 FSC매핑 "
                "시트 이력과 일치하면 자재코드 선택 창 목록 맨 위에 추천 "
                "코드를 표시합니다(세부공정까지 맞으면 확정⭐, 나머지만 "
                "맞으면 후보☆). 자재코드가 CIP AS-IS와 일치해 검토 필요로 "
                "잡히면 CIP에 등록된 TO-BE 교체 코드도 함께 추천합니다. "
                "Q-code 옆에 규격 검색창을 추가해 규격으로 Q-code를 찾아 "
                "입력할 수 있고, 옵션 입력창마다 개별 지우기(×) 버튼과 "
                "입력 중 자동 드롭다운도 추가했습니다.",
    },
    {
        "id": "2026-09-11-csp-fsc-map-writeback",
        "app": "csp",
        "title": "FSC매핑 시트 자동 갱신",
        "desc": "엑셀 파일(CSP 주문접수)을 생성하면, 이번에 쓴 조합 중 "
                "FSC매핑 시트에 없는 새로운 조합은 새 행으로 추가하고, "
                "이미 있는 조합에 이전과 다른 FSC를 썼으면 이력 열에 "
                "이어붙여 다음부터 새 값이 추천되게 합니다. 저장 전 "
                "원본을 Backup 폴더에 백업하고, 파일이 다른 프로그램에서 "
                "열려 있으면 안전하게 건너뜁니다.",
    },
    {
        "id": "2026-09-15-quote-output-slim",
        "app": "quote",
        "title": "견적서 파일 용량 대폭 감소",
        "desc": "생성되는 견적서에 단가표·코드매핑 등 내부 자료가 숨겨진 채로 "
                "같이 들어가던 것을 제거했습니다. 국내 견적서 기준 약 1MB에서 "
                "120KB로 줄어 생성·전달·열기가 모두 빨라지고, 대외비 파일에 "
                "내부 단가 자료가 실려 나가지 않습니다.",
    },
]
